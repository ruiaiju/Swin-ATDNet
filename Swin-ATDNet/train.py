...
if __name__ == "__main__":
    ...
    #                   自己需要的分类个数+1，如2+1(如果最后的分类结果只有猫和狗，那么这个值就是3，因为还有背景)
    #-----------------------------------------------------#
    num_classes     = 21
    #---------------------------------#
    #   所使用的的主干网络：
    ...
    Init_Epoch          = 0
    Freeze_Epoch        = 50
    Freeze_batch_size   = 8#如果爆显存的话，可以将这量设置的小一些。
    Freeze_lr           =5e-4#学习率
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Segmentation/train.txt"),"r",encoding="utf-8") as f:
        train_lines = f.readlines()
    with open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Segmentation/val.txt"),"r",encoding="utf-8") as f:
